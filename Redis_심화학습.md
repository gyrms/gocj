...
```
Redis 기본
1. Redis가 빠른 이유는 무엇인가요?
2. Redis의 영속성 옵션(RDB, AOF)에 대해 설명해주세요.
3. Redis가 싱글 스레드인데 왜 빠른가요?

자료구조
4. Redis 자료구조 종류와 각각의 사용 케이스를 설명해주세요.
5. ZSet(Sorted Set)을 어떻게 활용할 수 있나요?

캐싱
6. Redis 캐싱 전략(Look-Aside, Write-Through, Write-Behind)을 설명해주세요.
7. Cache Stampede 문제와 해결 방법을 설명해주세요.
8. 캐시 만료(TTL)를 어떻게 설계하시나요?

분산 락
9. Redis 분산 락을 왜 사용하나요? synchronized와 차이는?
10. Redisson Watchdog이 무엇인가요?

세션
11. Redis를 세션 저장소로 사용하는 이유는?
12. Redis 세션과 JWT 중 어떤 것을 선택하시겠어요?

운영
13. Redis 메모리가 부족해지면 어떻게 되나요?
14. Redis 장애 시 서비스가 중단되지 않으려면 어떻게 해야 하나요?
15. Redis와 Memcached의 차이는 무엇인가요?
```
